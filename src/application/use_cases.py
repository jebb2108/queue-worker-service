...
from src.domain.exceptions import MatchingException
...
class FindMatchUseCase:
    ...
    async def _select_best_candidates(self, user: User, candidates: List[User]) -> List[ScoredCandidate]:
        ...
        if not scored_candidates:
            logger.debug(
                f"No candidates passed threshold %s",
                str(config.matching.compatibility_threshold)
            )
            return []
        ...
        logger.debug(
            f"Selected %s candidates passing threshold, best score: %s",
            str(len(scored_candidates)), str(scored_candidates[0].score.total_score)
        )
...
class ProcessMatchRequestUseCase:
        ...
        try:
            ...
            async with uow:
                ...
                if match:
                    ...
                    try:
                        # Пытается закоммитить матч в базу данных
                        await uow.commit()

                        # Только после успешного коммита удаляем пользователей из очереди
                        ...
                        # Помечаем пользоваетелей нашедшими матч
                        await uow.states.update_state(user1_id, UserStatus.MATCHED)
                        await uow.states.update_state(user2_id, UserStatus.MATCHED)
                        ...
                        raise await self._schedule_retry(request, delay=2.0)
            ...
            logger.error(
                f"Exception in ProcessMatchRequestUseCase for user {request.user_id}: {e}"
            )
            ...
            await self.message_publisher.publish_to_dead_letter(
                request.to_dict(), f"Processing error: {str(e)}"  # noqa
            )
            return False


    async def _should_process_request(
        self, request: MatchRequest, uow: AbstractUnitOfWork
    ):
        ...
        if state:
            ...
            # Проверяет, если состояние пользователя изменилось
            if state.status in [UserStatus.CANCELED, UserStatus.MATCHED]:
                await uow.states.delete_state(request.user_id)
                return False

            # Проверяет, если состояние запроса пользователя - это отмена или матч
            elif UserStatus(request.status) in [UserStatus.CANCELED, UserStatus.MATCHED]:
                new_status = UserStatus(request.status)
                await uow.states.update_state(request.user_id, new_status)
                await uow.queue.remove_from_queue(request.user_id)
                return False

            # Проверить, не истек ли запрос
            elapsed = time.time() - state.created_at
            if elapsed >= config.matching.max_wait_time:
                await self._handle_timeout(request.user_id, None, uow, elapsed)
                return False

            # Ни одно условие не прошло - значит
            return True  # пользователь может продолжать поиск партнера

        else:
            # Проверить, находится ли пользователь в очереди
            if await uow.queue.is_searching(request.user_id):
                new_state = {
                    "user_id": request.user_id,
                    "status": UserStatus.WAITING,
                    "created_at": time.time(),
                }
                # Сохраняю состояние в оперативную память
    ...
    async def _handle_no_match(self, request: MatchRequest, uow: AbstractUnitOfWork):
        ...
        elapsed = datetime.now(tz=config.timezone) - request.created_at
        if (
            elapsed.total_seconds() >= config.matching.max_wait_time
            or request.retry_count >= config.matching.max_retries
        ):
    ...
    async def _handle_timeout(
        ...
        await self.metrics.record_user_status_change(
            UserStatus.WAITING, UserStatus.EXPIRED
        )
        logger.info("Time wait for message % s expired", user_id)

        # Очистить состояние и убрать из очереди
